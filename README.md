...
```
                ...
                (лазер / сварка / покраска)                  (промышленная телеметрия)
                          │                                            │
...
- **Телеметрия эксплуатации синтетическая** — реалистичная симуляция (`backend/ml/simulate_fleet.py`) с управляемой вероятностью деградации. В продакшене — реальный экспорт с с промышленных контроллеров телеметрии.
