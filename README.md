# СМАРТ ГВАРД (STARGUARD AI)
### AI-платформа сквозного контроля качества и предиктивной диагностики для ЭНЕРГОСТАР

Технический прототип (PoC) для позиции руководителя отдела инноваций и IT-разработок.
Показывает, как объединить два актива компании — металлообработку (выходной контроль)
и линейку СМАРТ ТМ/СМАРТ АВР (мониторинг в эксплуатации) — в единый ИИ-продукт с
измеримым влиянием на прибыль.

> ⚡ **Дисклеймер:** это не слайд с идеей, а работающий прототип — обученные нейросети,
> C++ движок реального времени, живой дашборд. Поднимается одной командой за 5 минут
> и говорит сам за себя лучше любой презентации. Синтетические только данные ОТК и
> телеметрии (реальных камер и датчиков под рукой не было) — код, модели и архитектура
> настоящие. Смотрите, запускайте, ломайте.

![Дашборд СМАРТ ГВАРД](docs/screenshot_dashboard.png)
...
в эксплуатации телеметрия СМАРТ ТМ/АВР анализируется потоковым C++ движком и
...
| СМАРТ ТМ/АВР продаются как разовое железо | Тот же поток данных превращается в сервис предиктивной диагностики | Новая статья recurring-revenue — платная подписка на мониторинг поверх уже проданного оборудования |
...
1. Пилот на одном производственном участке: подключить реальную камеру к посту
   контроля, дообучить CNN на реальных дефектах (transfer learning от текущей модели).
2. Подключить реальный экспорт данных СМАРТ ТМ/АВР с 5-10 объектов для калибровки
